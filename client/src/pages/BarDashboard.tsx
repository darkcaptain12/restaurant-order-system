...
export default function BarDashboard() {
  ...
  const fetchCompletedOrders = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        // Server tarafında zaten filtreleniyor, direkt kullan
  ...
  const handleWebSocketMessage = useCallback((data: any) => {
    if (data.type === 'NEW_ORDER' || data.type === 'ORDER_UPDATED' || data.type === 'ORDERS_MOVED_TO_COMPLETED') {
      fetchOrders();
      if (activeTab === 'completed') {
        setTimeout(() => fetchCompletedOrders(), 500);
      }
    }
          ...
          <div key={order.id} className="bg-white p-6 rounded-xl shadow-md border border-gray-200 hover:shadow-lg transition-shadow">
            ...
            <div className="space-y-3">
              {order.items.map(item => (
                <div
                  key={item.id}
                  className="flex justify-between items-center p-4 bg-gradient-to-r from-purple-50 to-white rounded-lg border border-gray-200"
                >
